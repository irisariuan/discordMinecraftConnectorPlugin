...
/**
 * Registers the {@code /cancelstop} command via Brigadier (classic Forge).
 * Requires the sender to be a player with at least OP level 2, or a console source.
 */
    ...
    public static void register(
        ...
        dispatcher.register(
            Commands.literal("cancelstop")
                .requires(source -> {
                    // Console / non-player sources always have access.
                    // Players must have at least permission level 2 (operator).
                ...
                .executes(ctx -> {
                    ...
                    ForgeShutdownManager shutdownManager = mod.getShutdownManager();

                    if (!shutdownManager.hasScheduledShutdown()) {
                        source.sendFailure(
                            Component.literal("No shutdown is currently scheduled")
